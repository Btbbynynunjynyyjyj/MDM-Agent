...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildUi() {
        ...
        val shield = TextView(this).apply {
            text = "◈"
            textSize = 28f
            setTextColor(Color.rgb(79, 140, 255))
            gravity = Gravity.CENTER
        }
        header.addView(shield, LinearLayout.LayoutParams(dp(42), dp(42)))
        ...
        titleBox.addView(TextView(this).apply {
            text = "Managed device"
            textSize = 12f
            setTextColor(Color.rgb(137, 154, 177))
        })
        ...
        search = EditText(this).apply {
            hint = "Dastur qidirish…"
            textSize = 15f
            singleLine = true
            setTextColor(Color.WHITE)
            setHintTextColor(Color.rgb(120, 137, 161))
            setPadding(dp(16), 0, dp(16), 0)
            background = rounded(Color.rgb(14, 28, 47), dp(14).toFloat())
            addTextChangedListener(SimpleTextWatcher { renderApps(it) })
        }
        val searchParams = LinearLayout.LayoutParams(-1, dp(48)).apply { topMargin = dp(10) }
        root.addView(search, searchParams)

        val update = Button(this).apply {
            text = "Barcha ilovalarni yangilash"
            textSize = 14f
            isAllCaps = false
            typeface = Typeface.DEFAULT_BOLD
            setTextColor(Color.WHITE)
            background = rounded(Color.rgb(55, 110, 220), dp(13).toFloat())
            setOnClickListener {
                Toast.makeText(this@MainActivity, "Yangilanishlar tekshirilmoqda…", Toast.LENGTH_SHORT).show()
            }
        }
        val updateParams = LinearLayout.LayoutParams(-1, dp(48)).apply { topMargin = dp(10) }
        root.addView(update, updateParams)
        ...
        val scrollParams = LinearLayout.LayoutParams(-1, 0, 1f).apply { topMargin = dp(14) }
    ...
    private fun loadAllowedApps() {
        ...
        allowedApps = allowedPackages.mapNotNull { pkg ->
            runCatching {
    ...
    private fun renderApps(query: String) {
        ...
        if (filtered.isEmpty()) {
            val empty = TextView(this).apply {
                text = "Ruxsat berilgan ilova topilmadi"
                textSize = 14f
                setTextColor(Color.rgb(137, 154, 177))
                gravity = Gravity.CENTER
            ...
            val params = GridLayout.LayoutParams().apply {
                width = -1
                height = dp(80)
                columnSpec = GridLayout.spec(0, columnCount)
            }
    ...
    private fun createAppCard(app: AppEntry): View {
        val card = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            gravity = Gravity.CENTER
            background = rounded(Color.rgb(13, 27, 45), dp(16).toFloat())
            ...
            setOnClickListener {
                ...
                else Toast.makeText(this@MainActivity, "Ilovani ochib bo‘lmadi", Toast.LENGTH_SHORT).show()
            }
        }
